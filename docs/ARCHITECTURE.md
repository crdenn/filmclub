...
The Plex token is encrypted before it is stored in the member row so the app can write that member's future ratings to Plex. Sessions are revocable and server-side: the cookie carries an opaque random token and the `sessions` table stores only its SHA-256 hash plus an expiry, so logout deletes the row and an admin can invalidate every session at once (`POST /api/admin/security/logout-all`). On each protected request, `current_member()` resolves the session to its member; a Plex-origin member with no decryptable token is treated as a legacy identity-only session and asked to sign in with Plex again to repopulate it, while local accounts are exempt. Login methods are recorded in the `identities` table (`plex`/`local`), and Plex login resolves through that table, so a member can authenticate with a local password, Plex, or both without creating a duplicate member. Local accounts are created only by redeeming a single-use, expiring, admin-issued invite (`invites` table, scrypt password in `app/passwords.py`). Admin-issued password resets use the same hash-only, expiring, single-use model in `password_resets`; redemption changes the scrypt hash and revokes the member's existing sessions. `members.plex_rating_sync_enabled` lets each member pause both outbound writes and inbound webhook application without deleting that connection. Token encryption derives from the durable data key (`config.DATA_KEY` — `SESSION_SECRET` if set, else the generated `master.key`), which is separate from the cookie-signing secret (`session.key`); rotating the data key requires members to sign in again, whereas rotating the signing secret only forces a re-login. `DEV_BYPASS_USER` short-circuits this flow and is development-only.
...
On an unconfigured database the app logs a one-time setup code and stores only a salted SHA-256 hash of it in `/data/setup_code` (the code expires after 30 minutes and wrong guesses are rate-limited to a rolling window; restart to reissue). The wizard first creates the local, non-demotable owner, then validates required TMDB configuration plus optional Plex and Seerr connections, stores sensitive values encrypted in `app_settings`, and marks setup complete. This permits a fresh installation with no Plex service. Admin settings reuse the same validation/storage layer. `POST /api/admin/settings/test` applies those checks to the effective unsaved form values but does not write them; `PUT /api/admin/settings` validates and persists. Explicit environment values take precedence and are shown read-only.
