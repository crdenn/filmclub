...
- **Consequences:** Raw filesystem backups must include the complete `/data` directory and `master.key`. The Admin panel's portable backup instead bundles the active data key with a consistent database snapshot and re-keys encrypted values during restore. That archive is sensitive and is intentionally not password-encrypted. The wizard creates the first local account as the non-demotable owner before integration setup; Plex is optional and its URL/token/machine-ID fields are all-or-none. Environment-overridden fields are read-only in the UI.
