...
_library = {
    ...
    "movies": {},     # ratingKey -> {rating_key, title, year, thumb, summary}
...
def random_movies(n: int) -> list[dict]:
    ...
    for m in _library["movies"].values():
...
def thumb_path(rating_key: str) -> str | None:
    ...
    """
    entry = _library["movies"].get(str(rating_key))
    if not entry:
        return None
